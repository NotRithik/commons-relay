...
class MessagingAdapter:
    SKILLS=frozenset(['messaging.send','messaging.join','messaging.create_group','storage.share'])
    ...
    def lookup(self,effect):
        ...
        if row['skill'] in ['messaging.join','messaging.create_group'] and row['result']:return Receipt(effect.reference,'confirmed',0,json.loads(row['result']))
        if row['result']:
        ...
        states=[self.mailbox.status(id)['state'] for id in ids]
