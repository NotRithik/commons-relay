...
public:
    static constexpr std::int64_t IntervalMs = 15000;
    static constexpr std::int64_t ReplyDeadlineMs = 20000;
    static constexpr std::int64_t StaleMs = 45000;
    ...
    const char* state(std::int64_t now) const {
        if (lastReply_ < 0) return now - lastAttempt_ > ReplyDeadlineMs ? "unconfirmed" : "checking";
